...
Module['FS_createPath']("/assets", "lunar-home", true, true);
Module['FS_createPath']("/assets/lunar-home", "tutorial", true, true);
Module['FS_createPath']("/assets/lunar-home/tutorial", "offline.gmi", true, true);
Module['FS_createPath']("/assets/lunar-home/tutorial", "backlog.gmi", true, true);
Module['FS_createPath']("/assets/lunar-home/tutorial", "subs.gmi", true, true);
Module['FS_createPath']("/assets/lunar-home/tutorial", "gemini.gmi", true, true);
Module['FS_createPath']("/assets/lunar-home/tutorial", "subscribe-to-me.gmi", true, true);
Module['FS_createPath']("/assets/lunar-home/tutorial/subscribe-to-me.gmi", "subs-nav-away.gmi", true, true);
Module['FS_createPath']("/assets/lunar-home/tutorial/subscribe-to-me.gmi", "subs-new-page.gmi", true, true);
...
function glue_preint(){ var entry = __glue_main_; if (entry) { if (navigator["gpu"]) { navigator["gpu"]["requestAdapter"]().then(function (adapter) { adapter["requestDevice"]().then( function (device) { Module["preinitializedWebGPUDevice"] = device; entry(); }); }, function () { console.error("No WebGPU adapter; not starting"); }); } else { console.error("No support for WebGPU; not starting"); } } else { console.error("Entry point not found; unable to start"); } }
  ...
  var WebGPU = {initManagers:function() {
        ...
        for (var i = 0; i < constantCount; ++i) {
          ...
          var key = UTF8ToString(HEAP32[(((entryPtr)+(4))>>2)]);
  ...
  function _js_close_application() {
      window.close();
    }

  function _js_copy_to_clipboard(clipboardPtr) {
  		// Permissions to write to clipboard.
  		// https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/Interact_with_the_clipboard
      var clipboardText = UTF8ToString(clipboardPtr);
  ...
  function _js_editor_focus() {
      window.lunar_editor.focus();
    }

  function _js_editor_force_layout() {
      window.lunar_editor.layout();
    }

  function _js_editor_get_text() {
      text = window.lunar_editor.getModel().getValue();
      byteLength = (lengthBytesUTF8(text) + 1);
      textPtr = Module._malloc(byteLength);
      stringToUTF8(text, textPtr, byteLength);
      return textPtr;
    }

  function _js_editor_set_text(cStringPtr) {
      var textToSet = UTF8ToString(cStringPtr);
      //window.lunar_editor.getModel().setEOL(0);
      window.lunar_editor.getModel().setValue(textToSet);
      window.lunar_editor.getModel().setEOL(0); // Note: this will update line endings.
      window.lunar_editor.getModel().updateOptions({ tabSize: 2 });
      // model.applyEdits([{ text: newValue, range: model.getFullModelRange()}]); }]
    }

  function _js_focus_element(cStringPtr) {
  ...
  function _js_gemini_request(request_url_ptr, fun_ptr, sequence) {
      if (_js_is_web()) {
        console.log("Gemini is not supported when using a browser.");
        return;
      }
  
      if (!_js_is_electron()) {
        console.log("Unknown platform: gemini is not supported.");
      }
  
      var request_url = UTF8ToString(request_url_ptr);
      // Note: This async function returns a promise that we ignore.
      let sendGeminiRequest = async () => {
        try {
          let response = await window.electron.geminiRequest(request_url);
          var ptr = allocate(intArrayFromString(response), ALLOC_NORMAL);
          _capi_handle_async_gemini_response(ptr, fun_ptr, sequence);
          _free(ptr);
        } catch (e) {
          let response = " " + e
          var ptr = allocate(intArrayFromString(response), ALLOC_NORMAL);
          _capi_handle_async_gemini_response_error(ptr, fun_ptr, sequence);
          _free(ptr);
        }
      };
      sendGeminiRequest();
  ...
  function _js_get_clipboard_text() {
      let getClipboardText = async () => {
        try {
          let response = await navigator.clipboard.readText();
          var ptr = allocate(intArrayFromString(response), ALLOC_NORMAL);
          _capi_handle_async_clipboard_text(ptr);
          _free(ptr);
        } catch (e) {
          console.log("Failed to get clipboard text");
        }
      };
      getClipboardText(); // IIFE might be more useful here. Need double parenthesis.
    }

  function _js_get_device_pixel_ratio() {
      var devicePixelRatio = 0; // Selected once and never updated.
      _js_get_device_pixel_ratio = function() {
  ...
  function _js_is_electron() {
      _js_is_electron = function() {
  ...
  function _js_is_web() {
      _js_is_web = function() {
        return !_js_is_electron();
      };
    }

  function _js_offer_download(filenamePtr, outputnamePtr, mimePtr) {
      var filename = UTF8ToString(filenamePtr);
      var outputname = UTF8ToString(outputnamePtr);
  ...
  function _js_open_using_system(urlPtr) {
      ...
      if (_js_is_electron()) {
        // Route to electron to display in the system browser.
        window.electron.systemOpenUrl(url);
      }
    }

  function _js_set_canvas_size(canvas_name_ptr, device_width, device_height, logical_width, logical_height) {
      var canvas_name = UTF8ToString(canvas_name_ptr);
      // See https://stackoverflow.com/questions/15661339/how-do-i-fix-blurry-text-in-my-html5-canvas/54027313#54027313
      var canvas = document.getElementById(canvas_name);
      if (!canvas)
      {
        alert("Failed to find canvas: " + canvas_name)
      }
      ...
      if (canvas.getContext("2d")) {
        let ratio = _js_get_device_pixel_ratio();
        canvas.getContext("2d").scale(ratio, ratio);
      }
    }

  function _js_set_up_indexeddb_filesystem() {
      // https://emscripten.org/docs/api_reference/Filesystem-API.html
      // home_path = FS.analyzePath('/lunar-home', true);
      // if (!home_path.exists) {
      FS.mkdir('/lunar-home');
      FS.mount(IDBFS, {}, '/lunar-home');
      FS.syncfs(true, function (err) {
        if (err) {
          console.log("syncfs: " + err);
        }
        assert(!err);
        // Notify app that home synchronization has taken place.
        _capi_filesystem_has_mounted();
      });
      //}
    }

  function _js_spartan_request(request_url_ptr, fun_ptr, sequence) {
      if (_js_is_web()) {
        console.log("Gemini is not supported when using a browser.");
        return;
      }
      ...
      var request_url = UTF8ToString(request_url_ptr);
      // Note: This async function returns a promise that we ignore.
      let sendSpartanRequest = async () => {
        try {
          let response = await window.electron.spartanRequest(request_url);
          console.log("AHere 5 -- " + response);
          var ptr = allocate(intArrayFromString(response), ALLOC_NORMAL);
          _capi_handle_async_spartan_response(ptr, fun_ptr, sequence);
          _free(ptr);
        } catch (e) {
          console.log("AHere 6");
          let response = " " + e
          var ptr = allocate(intArrayFromString(response), ALLOC_NORMAL);
          _capi_handle_async_spartan_response_error(ptr, fun_ptr, sequence);
          _free(ptr);
        }
      };
      sendSpartanRequest(); // IIFE might be more useful here. Need double parenthesis.
    }

  function _js_synchronize_indexeddb() {
  ...
  function _js_toggle_visibility(cStringPtr, visible) {
      var element_to_focus = UTF8ToString(cStringPtr);
      var element = document.getElementById(element_to_focus);
      if (element) {
        if (visible) {
  ...
  function _js_upload_local_file(sequence) {
  ...
  function _wgpuDeviceCreateRenderPipeline(deviceId, descriptor) {
      ...
      function makeBlendState(bsPtr) {
        ...
        assert(bsPtr);assert(HEAP32[((bsPtr)>>2)] === 0);
        return {
      ...
      function makeColorState(csPtr) {
        ...
        return {
          "format": WebGPU.TextureFormat[
            HEAPU32[(((csPtr)+(4))>>2)]],
          "blend": makeBlendState(HEAP32[(((csPtr)+(8))>>2)]),
      ...
      function makeMultisampleState(msPtr) {
        ...
        assert(msPtr);assert(HEAP32[((msPtr)>>2)] === 0);
        return {
      ...
      function makeFragmentState(fsPtr) {
        ...
        assert(fsPtr);assert(HEAP32[((fsPtr)>>2)] === 0);
        return {
          "module": WebGPU.mgrShaderModule.get(
            HEAP32[(((fsPtr)+(4))>>2)]),
          "entryPoint": UTF8ToString(
      ...
      var desc = {
        ...
        "depthStencil": makeDepthStencilState(
          HEAP32[(((descriptor)+(60))>>2)]),
        "multisample": makeMultisampleState(
          descriptor + 64),
        "fragment": makeFragmentState(
          HEAP32[(((descriptor)+(80))>>2)]),
      };
      var labelPtr = HEAP32[(((descriptor)+(4))>>2)];
      if (labelPtr) desc["label"] = UTF8ToString(labelPtr);
...
var asmLibraryArg = {
  ...
  "glue_preint": glue_preint,
  "js_close_application": _js_close_application,
  "js_copy_to_clipboard": _js_copy_to_clipboard,
  "js_editor_focus": _js_editor_focus,
  "js_editor_force_layout": _js_editor_force_layout,
  "js_editor_get_text": _js_editor_get_text,
  "js_editor_set_text": _js_editor_set_text,
  "js_focus_element": _js_focus_element,
  "js_gemini_request": _js_gemini_request,
  "js_get_clipboard_text": _js_get_clipboard_text,
  "js_get_device_pixel_ratio": _js_get_device_pixel_ratio,
  "js_is_electron": _js_is_electron,
  "js_is_web": _js_is_web,
  "js_offer_download": _js_offer_download,
  "js_open_using_system": _js_open_using_system,
  "js_set_canvas_size": _js_set_canvas_size,
  "js_set_up_indexeddb_filesystem": _js_set_up_indexeddb_filesystem,
  "js_spartan_request": _js_spartan_request,
  "js_synchronize_indexeddb": _js_synchronize_indexeddb,
  "js_toggle_visibility": _js_toggle_visibility,
  "js_upload_local_file": _js_upload_local_file,
...
/** @type {function(...*):?} */
var _capi_term_print = Module["_capi_term_print"] = createExportWrapper("capi_term_print");

/** @type {function(...*):?} */
var _capi_term_print_style = Module["_capi_term_print_style"] = createExportWrapper("capi_term_print_style");
...
/** @type {function(...*):?} */
var _capi_any_key_hit = Module["_capi_any_key_hit"] = createExportWrapper("capi_any_key_hit");

/** @type {function(...*):?} */
var _capi_handle_async_file_upload_failure = Module["_capi_handle_async_file_upload_failure"] = createExportWrapper("capi_handle_async_file_upload_failure");

/** @type {function(...*):?} */
var _capi_handle_async_file_upload = Module["_capi_handle_async_file_upload"] = createExportWrapper("capi_handle_async_file_upload");

/** @type {function(...*):?} */
var _capi_handle_async_spartan_response_error = Module["_capi_handle_async_spartan_response_error"] = createExportWrapper("capi_handle_async_spartan_response_error");

/** @type {function(...*):?} */
var _capi_handle_async_spartan_response = Module["_capi_handle_async_spartan_response"] = createExportWrapper("capi_handle_async_spartan_response");

/** @type {function(...*):?} */
var _capi_handle_async_gemini_response_error = Module["_capi_handle_async_gemini_response_error"] = createExportWrapper("capi_handle_async_gemini_response_error");

/** @type {function(...*):?} */
var _capi_handle_async_gemini_response = Module["_capi_handle_async_gemini_response"] = createExportWrapper("capi_handle_async_gemini_response");
...
/** @type {function(...*):?} */
var _capi_mouse_down = Module["_capi_mouse_down"] = createExportWrapper("capi_mouse_down");

/** @type {function(...*):?} */
var _capi_mouse_up = Module["_capi_mouse_up"] = createExportWrapper("capi_mouse_up");

/** @type {function(...*):?} */
var _capi_mouse_move = Module["_capi_mouse_move"] = createExportWrapper("capi_mouse_move");

/** @type {function(...*):?} */
var _capi_mouse_wheel = Module["_capi_mouse_wheel"] = createExportWrapper("capi_mouse_wheel");
...
/** @type {function(...*):?} */
var _capi_terminate_execution = Module["_capi_terminate_execution"] = createExportWrapper("capi_terminate_execution");

/** @type {function(...*):?} */
var _capi_handle_async_clipboard_text = Module["_capi_handle_async_clipboard_text"] = createExportWrapper("capi_handle_async_clipboard_text");

/** @type {function(...*):?} */
var _capi_do_tab = Module["_capi_do_tab"] = createExportWrapper("capi_do_tab");

/** @type {function(...*):?} */
var _capi_do_up_arrow = Module["_capi_do_up_arrow"] = createExportWrapper("capi_do_up_arrow");

/** @type {function(...*):?} */
var _capi_do_down_arrow = Module["_capi_do_down_arrow"] = createExportWrapper("capi_do_down_arrow");

/** @type {function(...*):?} */
var _capi_filesystem_has_mounted = Module["_capi_filesystem_has_mounted"] = createExportWrapper("capi_filesystem_has_mounted");

/** @type {function(...*):?} */
var __glue_main_ = Module["__glue_main_"] = createExportWrapper("_glue_main_");
...
unexportedRuntimeFunction('WebGPU', false);
unexportedRuntimeFunction('JsValStore', false);
